...
const benefitsData = [
  {
    icon: <Zap className="h-8 w-8 text-accent-foreground" aria-hidden="true" />,
  ...
  {
    icon: <ShieldCheck className="h-8 w-8 text-accent-foreground" aria-hidden="true" />,
  ...
  {
    icon: <RefreshCw className="h-8 w-8 text-accent-foreground" aria-hidden="true" />,
  ...
  {
    icon: <Paintbrush className="h-8 w-8 text-accent-foreground" aria-hidden="true" />,
...
const BenefitCard: React.FC<BenefitCardProps> = ({ icon, title, description, ariaLabel, index }) => (
    ...
    >
      <Card
        className="text-center shadow-lg hover:shadow-xl transition-shadow duration-300 border-border hover:border-primary/20 h-full flex flex-col bg-accent text-accent-foreground"
        ...
        <CardHeader className="flex flex-col items-center gap-4 pb-4">
          ...
          <CardTitle id={`benefit-title-${title.toLowerCase().replace(/\s+/g, '-')}`} className="text-xl font-semibold text-accent-foreground">{title}</CardTitle>
        </CardHeader>
        <CardContent className="flex-grow">
          <p id={`benefit-desc-${title.toLowerCase().replace(/\s+/g, '-')}`} className="text-accent-foreground/80 leading-relaxed">{description}</p>
        </CardContent>
      </Card>
    </motion.div>
);
...
export function Benefits() {
  return (
    <motion.section
        id="beneficios"
        className="bg-secondary" // Section background is Light Gray
